...
Generated files are tracked in `.power-dev-workflow/manifest.json`. Never replace an unmanaged file or a managed file whose content diverged from its recorded hash. Preserve existing `AGENTS.md` content outside the marked Power Dev Workflow block.
