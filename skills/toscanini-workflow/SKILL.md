...
Freeze exact changed-file paths in `validationScope.codeReviewPaths` and approved feature/screen QA scenarios. Dispatch only those targets to every verifier. QA may navigate other screens for setup, and reviewers may read dependencies for context; neither action authorizes an unrelated audit. Require scenario/path linkage and `changeEvidence` for in-contract findings. Never deliberately explore unrelated defects, even as follow-ups. Incidental observations remain non-blocking and receive no investigation or repair. A broader scope requires explicit owner approval. Canonical verification may run a broader existing suite; attribute failures to the current change before treating them as blockers.
